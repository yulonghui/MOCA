...
class Derpp(ContinualModel):
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            ...
            if not self.buffer.is_empty():
                ...
                if self.args.para_scale > 0:
                    loss += self.args.beta * 1.0 * self.loss(buf_outputs, buf_labels)
                    # loss += self.args.beta * 0.5 * ori_loss
                else:
                    loss += self.args.beta * ori_loss
        ...
        self.buffer.add_data(examples=not_aug_inputs[:real_batch_size],
                             ...
                             logits=outputs.data[:real_batch_size])
